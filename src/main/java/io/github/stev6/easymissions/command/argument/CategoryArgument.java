...
package io.github.stev6.easymissions.command.argument;
...
public record CategoryArgument(
        ConfigManager configManager) implements CustomArgumentType.Converted<@NotNull String, @NotNull String> {
    ...
    public String convert(String nativeType) throws CommandSyntaxException {
        if (!configManager.getMainConfig().categories().containsKey(nativeType.toLowerCase())) {
    ...
    public <S> @NotNull CompletableFuture<Suggestions> listSuggestions(
        ...
        configManager.getMainConfig().categories().keySet().stream()
