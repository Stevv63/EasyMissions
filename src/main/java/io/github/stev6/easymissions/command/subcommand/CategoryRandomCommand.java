...
import io.github.stev6.easymissions.command.argument.CategoryArgument;
...
public final class CategoryRandomCommand extends EasyMissionsCommand {
    ...
    public void addToTree(ArgumentBuilder<CommandSourceStack, ?> root) {
        root.then(Commands.literal(name)
                .then(Commands.argument("category", new CategoryArgument(plugin.getConfigManager()))
    ...
    public int execute(CommandContext<CommandSourceStack> ctx) throws CommandSyntaxException {
        ...
        for (Player target : targets)
            giveItem(ctx.getSource().getSender(), target, manager.categoryRandomMission(category), Source.RANDOM_CATEGORY);
